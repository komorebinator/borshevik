...
def ensure_venv():
    if not VENV_DIR.exists():
        print("🔧 Creating venv and installing gext...")
        subprocess.run([sys.executable, "-m", "venv", str(VENV_DIR)], check=True)

    print("🔄 Ensuring gnome-extensions-cli is up to date...")
    subprocess.run(
...
def is_installed(uuid):
    result = subprocess.run(
        [str(VENV_DIR / "bin/gext"), "list", "--enabled"],
...
def install(uuid):
    print(f"⬇️ Installing {uuid}")
    subprocess.run([str(VENV_DIR / "bin/gext"), "install", uuid], check=True)
    subprocess.run([str(VENV_DIR / "bin/gext"), "enable", uuid], check=True)
    mark_installed(uuid)
...
def main():
    ...
    for uuid in uuids:
        ...
        try:
            ...
            print(f"❌ Failed to install {uuid}")
